...
# 路径配置
# =============================
h5ad_dir = "/sibcb1/bioinformatics/yangyue/project/immunotherapy/1.TISCH_data_CD8T/H5ad_CD8T/"
qc_dir   = "/sibcb1/bioinformatics/yangyue/project/immunotherapy/7.2.cNMF_CD8T/h5ad_CD8T_QC/"
out_dir  = "/sibcb1/bioinformatics/yangyue/project/immunotherapy/7.2.cNMF_CD8T/cnmf_CD8T_out/"
...
# cNMF 参数
# =============================
Ks = np.arange(5, 11)          # K 候选
n_iter = 20                    # 用于 K 选择的迭代次数
final_n_iter = 200             # 最终模型迭代次数
seed = 14
...
for h5ad_file in sorted(os.listdir(h5ad_dir)):
    ...
    plt.title(f"{dataset} cNMF K selection")
    plt.legend()
    plt.tight_layout()
    plt.savefig(
    ...
    # -----------------------------
    # 5. exploratory run 下的 consensus（可选）
    # -----------------------------
    cnmf_obj.consensus(
        k=best_k,
        density_threshold=density_threshold,
    ...
    # -----------------------------
    # 6. Final cNMF：仅 K*，200 iterations
    # -----------------------------
    ...
    cnmf_final.prepare(
        counts_fn=qc_file,
        components=[best_k],      # ★ 只用最优 K*
        n_iter=final_n_iter,      # ★ 200 次迭代
        seed=seed,
        num_highvar_genes=num_highvar_genes
    )
    ...
    print(f"🎯 Final cNMF finished for {dataset} (K*={best_k}, 200 iters)")