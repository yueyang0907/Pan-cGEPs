...
sample_files <- list.files(
  ...
  pattern = "_seurat_Mono_Macro.rds$",
  full.names = TRUE
)

usage_data <- list.files(
  path = "/sibcb1/bioinformatics/yangyue/project/immunotherapy/7.4.cNMF_Mono_Macro/4.3.Mono_Macro_GEP_usage/",
...
# 3. 输出目录
# =============================
outdir <- "/sibcb1/bioinformatics/yangyue/project/immunotherapy/7.4.cNMF_Mono_Macro/4.3.2.Mono_Macro_usage_immu_pheatmap/"
...
for (file in sample_files) {
  ...
  # 提取样本名
  sample_name <- sub("_seurat_Mono_Macro$", "", file_path_sans_ext(basename(file)))
  message("------------------------------------------------")
...
write.csv(
  stat_df,
  file.path(outdir, "4.3.2.Mono_Macro_cGEP_usage_cell_patient_stats.csv"),
  row.names = FALSE
)
